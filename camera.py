...
class Camera:
    ...
    def __init__(self, controller):
        ...
        self.at = np.array([0.0, 0.0, -0.5]) # centro de movimiento de la camara y donde mira la camara
        ...
        self.autoCurve = cv.evalMultiCatCurve([[0, 0, 4], [10, 0, 4],[3,0,-0.5], [0.0, 5.0, 4], [-10.0, 0, 4], [-4, 0, -0.5], [0.0, -5.0, 4], [10, 0, 4],\
                                                 [0, 0, 4]], self.N)
    ...
    def update_view(self, delta):
        ...
        if self.controller.manual == True:
            self.eye[0] = self.manualCurve[math.floor(self.index) % self.N][0]
            self.eye[1] = self.manualCurve[math.floor(self.index) % self.N][1]
            self.eye[2] = self.manualCurve[math.floor(self.index) % self.N][2]
