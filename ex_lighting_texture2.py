...
if __name__ == "__main__":
    ...
    #plano con texturas
    shapePlane = createTexturePlane()
    gpuPlane = createGPUShape(textureGouraudPipeline, shapePlane)
    gpuPlane.texture = es.textureSimpleSetup(
        getAssetPath("plan.png"), GL_REPEAT, GL_REPEAT, GL_LINEAR, GL_LINEAR)
    ...
    #primer dado#
    #############
    vueltas_x_d1 = rand.randint(1,4)
    vueltas_y_d1 = rand.randint(1,4)
    pos_z_d1 = 5

    #v_rotacion_y1
    v_caida = 0.1
    v_rotacion_x1 = (pi/2*vueltas_x_d1)/((pos_z_d1/v_caida))
    v_rotacion_y1 = (pi/2*vueltas_y_d1)/((pos_z_d1/v_caida))
    rot_x_d1 = 0
    rot_y_d1 = 0
    rot_z_d1 = 0
    ...
    #segundo dado#
    ##############
    vueltas_x_d2 = rand.randint(0,6)
    vueltas_y_d2 = rand.randint(0,6)
    pos_z_d2 = 5

    #v_rotacion_y1
    v_caida = 0.1
    v_rotacion_x2 = (pi/2*vueltas_x_d2)/((pos_z_d2/v_caida))
    v_rotacion_y2 = (pi/2*vueltas_y_d2)/((pos_z_d2/v_caida))
    rot_x_d2 = 0
    rot_y_d2 = 0
    rot_z_d2 = 0
    ...
    while not glfw.window_should_close(window):
        ...
        vel_total1 = rand.randint(65,75)
        vel_total2 = rand.randint(65,75)

        #Movimiento de los dados
        if pos_z_d1 > 0: # todo se detiene cuanto los dados tocan el suelo
            pos_z_d1 -= v_caida*dt*vel_total1
            rot_x_d1 += v_rotacion_x1*dt*vel_total1
            rot_y_d1 += v_rotacion_y1*dt*vel_total1
        else:
            pos_z_d1 = 0

        if pos_z_d2 > 0: # todo se detiene cuanto los dados tocan el suelo
            pos_z_d2 -= v_caida*dt*vel_total2
            rot_x_d2 += v_rotacion_x2*dt*vel_total2
            rot_y_d2 += v_rotacion_y2*dt*vel_total2
        else:
            pos_z_d2 = 0
        ...
        # Drawing
        transformacion_dado1 = tr.matmul([tr.translate(0.85,0,pos_z_d1),tr.rotationX(rot_x_d1),tr.rotationY(rot_y_d1)])
        glUniformMatrix4fv(glGetUniformLocation(lightingPipeline.shaderProgram, "model"), 1, GL_TRUE, transformacion_dado1)
        lightingPipeline.drawCall(gpuDice)

        transformacion_dado2 = tr.matmul([tr.translate(-0.75,0,pos_z_d2),tr.rotationX(rot_x_d2),tr.rotationY(rot_y_d2)])
        glUniformMatrix4fv(glGetUniformLocation(lightingPipeline.shaderProgram, "model"), 1, GL_TRUE, transformacion_dado2)
        lightingPipeline.drawCall(gpuDiceBlue)

        glUniformMatrix4fv(glGetUniformLocation(lightingPipeline.shaderProgram, "model"), 1, GL_TRUE, tr.matmul([tr.translate(0,0,-0.5),tr.scale(10,10,0)]))
        lightingPipeline.drawCall(gpuPlane)
